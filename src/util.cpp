...
const char * const NEOM_CONF_FILENAME = "neom.conf";
const char * const NEOM_PID_FILENAME = "neomd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "neomd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NEOM
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NEOM
    // Mac: ~/Library/Application Support/NEOM
    // Unix: ~/.neom
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NEOM";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NEOM";
#else
    // Unix
    return pathRet / ".neom";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", NEOM_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", NEOM_PID_FILENAME)));
}
