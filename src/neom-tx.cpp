...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " neom-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  neom-tx [options] <hex-tx> [commands]  Update hex-encoded neom transaction\n" +
            "or:     neom-tx [options] -create [commands]   Create hex-encoded neom transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded neom transaction
