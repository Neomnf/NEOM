...
###   neomcore.org and neom.org and compares them.
...
WORKINGDIR="/tmp/neom_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://neomcore.org"
HOST2="https://neom.org"
BASEDIR="/bin/"
VERSIONPREFIX="neom-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'neom-',
...
if [ $? -ne 0 ]; then
   echo "neom.org failed to provide signature file, but neomcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "neom.org and neomcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the NEOM Core binary release signing key installed?"
   fi
